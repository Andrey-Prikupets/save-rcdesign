...
app.get('/prn', function(req, res) {
	if (INFO) console.log('Query received\n', req.query);

	var startURL = req.query.url;
	if (INFO) console.log('url=', startURL);
	...
	function processURL(pageURL, html, e) {
		if (e || html == null) {
    			console.log("ERROR: Error getting <B>"+pageURL+"</B><BR>"+e);
			res.end(String(e));
			return;
		}
		...
		pagesCount++;
    		if (INFO) console.log("processURL, Processing "+pagesCount+" page ("+html.length+" bytes): "+pageURL); 
		...
		try {
			if (isBlog) {
    			...
    			console.log("Parsing error at <B>"+pageURL+"</B><BR>"+e);
			res.end(String(e));
			return;
		}
		...
		function completePage(pageURL, html, e) {
			if (e || html == null) {
    				console.log("ERROR: Parsing error at <B>"+pageURL+"</B><BR>"); 
			...
			var pageStat = processPages(html);
				...
				return;
    				...
    				if (INFO) console.log("processURL, completePage: "+pagesCount+" pages read"); 
...
function getURL(pageURL, callback) {
       if (INFO) console.log('getURL: ' + pageURL);
       http.get(pageURL, function(resOther) {
       	if (DEBUG) console.log('STATUS: ' + resOther.statusCode);
       	if (DEBUG) console.log('HEADERS: ' + JSON.stringify(resOther.headers));
       	var data = [];
       	var i = 0;
       	var totallength = 0;
       	...
       	resOther.on("end", function() {
       		...
       		if (DEBUG) console.log("Received "+data.length+" chunks of total "+totallength+" bytes");
       		var pos = 0;
...
function processPages(html) {
	...
	if (endPage2 < 0) return null;
        ...
        if (DEBUG ) console.log ("Pages="+pageRangeStart+".."+pageRangeEnd);
	...
	if (endPage2 < 0) return null;
        ...
        if (DEBUG) console.log ("recordsNum="+recordsNum);
	...
	if (startNextPage < 0) return null;
        ...
        if (DEBUG) console.log ("nextPageURL="+nextPageURL); 	
...
function processBlog(startURL, html, completeEntryCallBack, completePageCallBack) {
	...
	while (true) {
        	...
        	if (DEBUG) console.log ("entryURL="+entryURL);
        	...
        	if (DEBUG) console.log ("entryTitle="+entryTitle);
        ...
        if (DEBUG) console.log ("entries.length="+entries.length);

	iterate();

	function iterate() {
		if (entries.length == 0) {
        		if (DEBUG) console.log ("processBlog, no more entries.");
		...
		function handleBlogEntry(pageURL, htmlEntry, e) {	
			...
			var acc = [];
        		if (DEBUG) console.log ("processBlog, processBlogEntry, pageURL="+pageURL);
...
function processBlogEntry(acc, html, e) {
	...
	var endTitle = findStr(html, '<', startTitle);
        ...
        if (startClass < 0) return null;
       	...
       	if (DEBUG) console.log ("memberURL="+memberURL);
	...
	text = replaceIFrames(text);
        ...
        if (TRACE) console.log ("text="+text);
...
//        if (DEBUG) console.log ("threadURL="+threadURL);
...
//        var endThreadName = findStr(html, '</a>', endA);
        ...
        if (DEBUG) console.log ("threadName="+threadName);
...
function processForumThread(startURL, html, first, completeEntryCallBack, completePageCallBack) {
	...
	while (true) {
		...
		if (first) {
        	...
        	if (DEBUG) console.log ("entryDate="+entryDate);
        	...
        	if (DEBUG) console.log ("entryTime="+entryTime);
        	...
        	if (DEBUG) console.log ("postCount="+postCount);
...
function findStr(s, pattern, start) {
	...
	if (TRACE) console.log("findStr at "+start+": "+pattern+" => "+pos);
	return pos;
}
...
function findStrAfter(s, pattern, start) {
	...
	if (pos >= 0)
		pos += pattern.length;
	if (TRACE) console.log("findStrAfter at "+start+": "+pattern+" => "+pos);
	return pos;
}

function subStr(s, start, end) {
	var str = s.substr(start, end-start);
	if (TRACE) console.log("subStr "+start+".."+end+" => "+str);
	return str;
}
...
function makeFileName(url, ext) {
	if (TRACE) console.log ("makeFileName: "+url);
	...
	if (ext != null) {
		var extFound = false;
		for (extStr in ext) {
			if (TRACE) console.log ("makeFileName, extStr="+ext[extStr]);
	...
	if (DEBUG) console.log ("makeFileName fileName="+url);
	return url;
}
